...
public class SkinInflaterFactory implements Factory {
	...
	private void parseSkinAttr(Context context, AttributeSet attrs, View view) {
		...
		for (int i = 0; i < attrs.getAttributeCount(); i++){
			...
			if(!SupportAttributes.isSupport(attrName)){
				continue;
			}
		    ...
		    if(attrValue.startsWith("@")){
				try {
					...
					SkinAttr mSkinAttr = AttrFactory.get(attrName, id, entryName);
					if (mSkinAttr != null) {
	...
	public void dynamicAddSkinEnableView(Context context, View view, List<DynamicAttr> pDAttrs){	
		...
		for(DynamicAttr dAttr : pDAttrs){
			...
			String entryName = context.getResources().getResourceEntryName(id);
			String typeName = context.getResources().getResourceTypeName(id);
			SkinAttr mSkinAttr = AttrFactory.get(dAttr.attrName, id, entryName);
			viewAttrs.add(mSkinAttr);
		}
